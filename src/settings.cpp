...
public:
    SettingsWindow()
    {
        ...
        auto *intro = new QLabel(tr("One scroll speed for all listed apps. Other apps stay unchanged."));
        ...
        connect(m_speed, &QSlider::valueChanged, this, [this](int value) {
            m_percentage->setText(tr("%1% of KDE scroll speed (100% = unchanged)").arg(value));
        });
        ...
        connect(pick, &QPushButton::clicked, this, [this, pick] { pickWindow(pick); });
        connect(manual, &QPushButton::clicked, this, [this] {
            bool ok = false;
            const auto id = QInputDialog::getText(this, tr("Add app ID"),
                tr("Exact Wayland app ID (for example brave-browser, chromium, or google-chrome):"), QLineEdit::Normal, {}, &ok);
            if (ok) {
                addApp(id);
            }
        });
        connect(remove, &QPushButton::clicked, this, [this] { delete m_apps->takeItem(m_apps->currentRow()); });

        m_status = new QLabel(tr("Apply saves settings and enables the installed plugin for this session."));
        // App IDs and D-Bus errors land here; QLabel would auto-detect them as rich text.
        ...
        connect(disable, &QPushButton::clicked, this, [this] {
            ...
            if (unloadPlugin()) {
                m_status->setText(wasLoaded ? tr("Disabled for this session. Stock scrolling restored.")
                                            : tr("Plugin was not loaded. Nothing to disable."));
            }
        });

        const KConfigGroup config(KSharedConfig::openConfig(QStringLiteral("scrollfixrc")), QStringLiteral("Scroll"));
        const auto ids = config.readEntry("Applications", QStringList{});
        for (const auto &id : ids) {
            addApp(id);
        }
        const auto factor = config.readEntry("Factor", 1.0);
        m_speed->setValue(ScrollFix::validFactor(factor) ? qRound(factor * 100) : 100);
    }
...
private:
    ...
    {
        m_status->setText(tr("Click inside the target app, not its taskbar icon. Escape cancels."));
        ...
        auto *watcher = new QDBusPendingCallWatcher(kwin.asyncCall(QStringLiteral("queryWindowInfo")), this);
        connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, button](QDBusPendingCallWatcher *call) {
            const QDBusPendingReply<QVariantMap> reply = *call;
            button->setEnabled(true);
            call->deleteLater();
            if (reply.isError()) {
                m_status->setText(tr("Window selection failed: %1").arg(reply.error().message()));
                return;
            }
            const auto id = reply.value().value(QStringLiteral("resourceClass")).toString();
            if (id.isEmpty() || id == QStringLiteral("plasmashell")) {
                m_status->setText(tr("No application selected. Pick inside the application's window."));
                return;
            }
            addApp(id);
            m_status->setText(tr("Added %1. Click Apply to activate. Works with Chromium-based and other native Wayland apps.").arg(id));
        });
    }
    ...
    bool pluginLoaded()
    {
        QDBusInterface plugins(QStringLiteral("org.kde.KWin"), QStringLiteral("/Plugins"),
    ...
    bool unloadPlugin()
    {
        QDBusInterface plugins(QStringLiteral("org.kde.KWin"), QStringLiteral("/Plugins"),
                               QStringLiteral("org.kde.KWin.Plugins"), QDBusConnection::sessionBus());
        plugins.setTimeout(3000);
        const QDBusReply<void> reply = plugins.call(QStringLiteral("UnloadPlugin"), pluginId);
        if (!reply.isValid()) {
            m_status->setText(tr("Could not disable plugin: %1").arg(reply.error().message()));
        }
    ...
    void applySettings()
    {
        ...
        if (!config->sync()) {
            QMessageBox::warning(this, tr("Save failed"), tr("Could not save settings. Running plugin was not changed."));
            return;
        }
        if (!unloadPlugin()) {
            return;
        ...
        QDBusInterface plugins(QStringLiteral("org.kde.KWin"), QStringLiteral("/Plugins"),
                               QStringLiteral("org.kde.KWin.Plugins"), QDBusConnection::sessionBus());
        plugins.setTimeout(3000);
        const QDBusReply<bool> reply = plugins.call(QStringLiteral("LoadPlugin"), pluginId);
        if (!reply.isValid() || !reply.value()) {
            m_status->setText(tr("Settings saved, but plugin loading failed. Check installation and KWin version. %1")
                                  .arg(reply.isValid() ? QString{} : reply.error().message()));
            return;
        }
        m_status->setText(tr("Applied: %1% for %2 apps. Startup loading was not changed.")
                              .arg(m_speed->value()).arg(ids.size()));
    }
